...
**Data:** 28/06/2026
**Ambiente:** Node 20.19.5, npm 10.8.2 · commit analisado `e578cfb`
**Ferramentas:** `npm audit` (SCA — dependências) e SonarQube Community 26.6.0 (SAST — código)
...
| Code Smells | 39 | **A** (Maintainability) |
| Cobertura | 80,0% | — |
| Duplicação | 0,0% | — |
| Linhas de código | 5.691 | — |
| **Quality Gate** | **PASSED** | — |

**Leitura:** o código não introduz vulnerabilidades nem *security hotspots* próprios. Os 39
*code smells* são de manutenibilidade (rating A), não de segurança. A cobertura de 80,0%
medida pelo Sonar abrange **todo** o `src/` (inclusive controllers, adapters e módulos); os
domínios críticos (`dominio/` + `aplicacao/`) — foco do *threshold* de 80% do projeto — estão
em ~98%, com o restante das linhas vindo das camadas de I/O (cobertas pelos testes e2e).

> Como reproduzir: ver o **Anexo — Comandos executados** ao final (sobe o SonarQube via
> Docker, gera a cobertura e roda o scanner).
...
**Validação da remediação:** suíte de **200 testes unitários** (domínio + aplicação)
reexecutada em 28/06/2026 com `npm run test:cov` — todos passando, sem regressões; o
SonarQube importou a cobertura desse mesmo run (cobertura global 80,0%).
...
# SAST (código) — SonarQube Community self-hosted via Docker
